...
/**
 * Class for ROS2 publish /local_plan topic to robot
 * @see rclnodejs.Publisher
 */
export default class LocalPlanPublisher implements Publisher {
    ...
    start(): void {
        ...
        this.mqtt.subscribe('wavem/1/local_plan');
        this.mqtt.client.on("message", (topic, message) => {
            try {
                log.info(`RCL local_plan publish MQTT onMessage topic : ${topic}, message : ${message}`);
                if(topic.includes('local_plan')) {
                    let msg = this.genPathMsg(message.toString());
                    publisher.publish(msg);
                } else return;
            } catch (error) {
                log.error(`RCL publish local_plan error : ${error}`);
            }
        });
    };
    ...
    stop(): void {
        ...
        this.mqtt.client.unsubscribe('wavem/1/local_plan');
        this.node.destroy();
    };
